...
dd("asd");
    ...
    public function prekg()
    {
        ...
        $data = Crud::where('web_page', '11')->paginate(50);
        $dataText = Post::where('page', '11')->paginate(50);

        return view('pages.prekg', compact('data', 'dataText'))
            ->with('i', (request()->input('page', 1) - 1) * 50);
    ...
    public function prekgmang(){
        $data = Staff::where('branch', '16')->paginate(150);
        return view('pages.prekgmang', compact('data'))
            ->with('i', (request()->input('page', 1) - 1) * 150);
